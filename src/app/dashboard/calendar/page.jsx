            ...
            <Dialog key={index}>
              <DialogTrigger asChild>
                <div className="h-24 bg-white border rounded-lg p-2 cursor-pointer hover:bg-gray-50 overflow-hidden">
                  ...
                  {dayTasks.slice(0, 2).map(task => (
                    <div key={task.id} className={`text-xs truncate mt-1 p-1 rounded ${getPriorityColor(task.priority)}`}>
                      {task.title}
                    </div>
                  ))}
                  {dayTasks.length > 2 && (
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-4">
                  ...
                  <Button onClick={() => { setSelectedDate(date); addTask(); }} className="w-full">
                      ...
                      <div key={task.id} className={`p-2 rounded ${getPriorityColor(task.priority)}`}>
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-xs mt-1">Deadline: {task.deadline.toLocaleDateString()}</p>
                          </div>
